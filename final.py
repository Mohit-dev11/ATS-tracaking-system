...
# Sidebar Navigation
option = st.sidebar.radio("Select an Option", ["Resume Analysis", "Build Your Own Resume", "Job Search Recommendations", "Cover Letter Generator"])
...
# Job Search Recommendations Section
    ...
    with st.form("job_search_form"):
        ...
        skills = st.text_area("Skills", placeholder="e.g., Python, JavaScript, SQL")
    ...
    if submit:
        if job_title and skills:
            linkedin_search_url = f"https://www.linkedin.com/jobs/search/?keywords={job_title}&location={location}"
            st.write(f"Job listings for **{job_title}** with skills **{skills}** in **{location}**.")
            st.markdown(f"[Click here to search for jobs on LinkedIn]({linkedin_search_url})")
        else:
